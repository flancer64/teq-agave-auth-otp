...
const config = {
    getLocal: () => ({urlBase: 'example.com'})
};

describe('Fl64_Auth_Otp_Back_Email_SignUp_Verify.perform', () => {
    ...
    async function createTestContainer(extraMocks = {}) {
        ...
        // Always needed
        container.register('TeqFw_Core_Back_Config$', config);
    ...
    describe('use cases', () => {
        it('should return SUCCESS if email is sent', async () => {
            ...
            const email = await container.get('Fl64_Auth_Otp_Back_Email_SignUp_Verify$');
